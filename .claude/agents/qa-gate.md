...
## Controlli obbligatori su `pipeline/draft.md`
...
5. **Disclosure coerente**: se `pipeline/affiliate-log.json` mostra `links_inserted` non vuoto, verifica che lo shortcode di disclosure sia presente nel testo. Se mancante, è un blocco critico (rischio legale).
...
## Output
Scrivi `pipeline/qa-result.json`:
